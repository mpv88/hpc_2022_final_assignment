...
static int parse_integer(const char *value, int *result)
{
    ...
    if (errno != 0 || end == value || *end != '\0' || number < INT_MIN || number > INT_MAX)
        return -1;
    *result = (int)number;
    return 0;
}
    ...
    *args = (arguments_t){
    ...
    while ((c = getopt(argc, argv, "irk:w:h:e:f:n:s:")) != -1) {
        ...
        case 'r':
            ...
            break;
        case 'k':
            if (size_set || width_set || height_set) return argument_error(args, "-k cannot be combined with -w or -h");
            if (parse_integer(optarg, &args->width) != 0 || args->width < 100) return argument_error(args, "playground size must be at least 100");
            args->height = args->width;
            size_set = 1;
            break;
        case 'w':
            if (size_set) return argument_error(args, "-w cannot be combined with -k");
            if (parse_integer(optarg, &args->width) != 0 || args->width < 100) return argument_error(args, "width must be at least 100");
            width_set = 1;
            break;
        case 'h':
            if (size_set) return argument_error(args, "-h cannot be combined with -k");
            if (parse_integer(optarg, &args->height) != 0 || args->height < 100) return argument_error(args, "height must be at least 100");
            height_set = 1;
            break;
        case 'e':
